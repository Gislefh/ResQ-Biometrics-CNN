...
def softmax_cross_entropy(pred_fake, pred_truth, labels):
    truth = tf.nn.softmax_cross_entropy_with_logits_v2(labels=labels, logits=pred_truth)
    fake = tf.nn.softmax_cross_entropy_with_logits_v2(labels=labels, logits=pred_fake)
    softmax = truth + fake
    return softmax
